...
import {
  ...
  editDirectorValidation,
} from "../validation/directorValidation.js";
...
export async function getAllDirectors(req, res) {
  try {
...
export async function getDirector(req, res) {
  try {
    ...
    if (!director)
      return res
        .status(404)
        .json({ status: 404, message: "Director not found" });
...
export async function getDirectorMovies(req, res) {
  try {
    ...
    const totalMovies = await Movies.countDocuments({ director: director._id });
...
export async function getDirectorSeries(req, res) {
  try {
    const director = await Director.findById(req.params.id).select("fullName");
    if (!director)
      return res
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (req.body.profile && director.profile) {
        unlinkSync(join(__dirname, "../public/director/", director.profile));
      }
