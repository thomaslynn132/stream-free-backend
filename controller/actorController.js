...
import {
  ...
  editActorValidation,
} from "../validation/actorValidation.js";
...
//! Get Request
export async function allActors(req, res) {
  try {
...
export async function getActorMovies(req, res) {
  try {
    ...
    const totalMovies = await Movies.countDocuments({ actors: actorIdStr });
...
export async function getActorSeries(req, res) {
  try {
    const actor = await Actor.findById(req.params.id).select("fullName");
    if (!actor)
      return res.status(404).json({ status: 404, message: "Actor not found" });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!actor)
        return res
          .status(404)
          .json({ status: 404, message: "Actor not found" });

      if (req.body.profile && actor.profile) {
        unlinkSync(join(__dirname, "../public/actor/", actor.profile));
      }
      ...
      res
        .status(200)
        .json({ status: 200, message: "Actor updated", actor: updatedActor });
    } catch (error) {
      res.status(500).json({ status: 500, message: error.message });
    }
  },
